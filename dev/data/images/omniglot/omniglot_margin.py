...
class OMNIGLOT_MARGIN(data.Dataset):
    ...
    '''
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, partition=0.8, omniglot_loader=None, classes=3, episode_size=30, scenario=False, scenario_size=5, margin_time=2, q_network=None):
    ...
    def __getitem__(self, index):
        if self.scenario:
            ...
            if self.train:
                # Train-dataset:
                img_classes = np.random.choice(len(self.train_labels), int(self.classes*2), replace=False)
