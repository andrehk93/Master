...
### CLASSES ###
from utils import loss_plot, fast_scatterplot as scatterplot, transforms, loader, logger
...
from reinforcement_utils import test, train_fast as train
...
# Checkpoint Loader:
parser.add_argument('--load-checkpoint', default='pretrained/reinforced_lstm_target_3/checkpoint.pth.tar', type=str,
                    help='path to latest checkpoint (default: none)')

# Network Name:
parser.add_argument('--name', default='reinforced_lstm_target_3', type=str,
                    help='name of file')
...
def update_dicts(stats, req_dict, acc_dict):
    new_req_dict = stats[0]
    new_acc_dict = stats[1]

    for key in req_dict.keys():
        req_dict[key].append(new_req_dict[key])
        acc_dict[key].append(new_acc_dict[key])
...
if __name__ == '__main__':
    ...
    # Parameters for target network and checkpoint saving:
    ...
    while not done:
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            ...
            stats = train.train(q_network, target_network, epoch, optimizer, \
                                train_loader, args, rl, episode, criterion, update, NTM)

            episode += args.batch_size

            # STATS:
            total_prediction_accuracy.append(stats[0])
            total_accuracy.append(stats[1])
            total_requests.append(stats[2])
            total_loss.append(stats[3])
            total_reward.append(stats[4])

            update_dicts(stats[5:], req_dict, acc_dict)
    ...
    if (val.lower() == "y"):
        ...
        for epoch in range(args.epochs + 1, args.epochs + 1 + test_epochs):
            ...
            # Validate the model:
            test_set_stats = test.validate(q_network, epoch, optimizer, test_loader, args, rl, episode, criterion)

            # Increment episode count:
            episode += args.batch_size

            # Statistics:
            test_accuracy += test_set_stats[0]
            test_request += test_set_stats[1]
            test_reward += test_set_stats[2]

            # For stat file:
            test_stats[0].append(test_set_stats[0])
            test_stats[1].append(test_set_stats[1])

            # Statistics:
            total_accuracy.append(test_set_stats[0])
            total_requests.append(test_set_stats[1])
            total_reward.append(test_set_stats[2])

            # Instance accuracies/requests:
            update_dicts(test_set_stats[3:], req_dict, acc_dict)
