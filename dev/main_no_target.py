...
# Checkpoint Loader:
parser.add_argument('--load-checkpoint', default='pretrained/reinforced_lrua_test/checkpoint.pth.tar', type=str,
                    help='path to latest checkpoint (default: none)')

# Checkpoint Loader:
parser.add_argument('--load-best-checkpoint', default='pretrained/reinforced_lrua_test/best.pth.tar', type=str,
                    help='path to best checkpoint (default: none)')

# Checkpoint Loader:
parser.add_argument('--load-test-checkpoint', default='pretrained/reinforced_lrua_test/testpoint.pth.tar', type=str,
                    help='path to best checkpoint (default: none)')

# Network Name:
parser.add_argument('--name', default='reinforced_lrua_test', type=str,
                    help='name of file')
...
if __name__ == '__main__':
    ...
    if (not MNIST_TEST):
        ...
        OMNIGLOT(root, train=False, transform=test_transform, omniglot_loader=omniglot_loader, episode_size=args.episode_size, test=True),
        batch_size=args.test_batch_size, shuffle=True, **kwargs)

        test_train_loader = torch.utils.data.DataLoader(
        OMNIGLOT(root, train=True, transform=test_transform, download=True, omniglot_loader=omniglot_loader, episode_size=args.episode_size, test=True),
        batch_size=args.test_batch_size, shuffle=True, **kwargs)
    else:
        root = 'data/images/mnist'
        test_loader = torch.utils.data.DataLoader(
        MNIST(root, transform=test_transform, episode_size=args.episode_size, classes=args.class_vector_size),
        batch_size=args.test_batch_size, shuffle=True, **kwargs)
