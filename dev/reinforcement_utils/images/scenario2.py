...
def run(model, scenario_loader, batch_size, reinforcement_learner, class_vector_size, cuda, multi_state=False, state_size=5):
    ...
    for i_e in range(len(label_batch)):
        ...
        requests.append(float(reqs/total))


        # Observe next state and images:
        if (multi_state):
            next_state_start = reinforcement_learner.next_multistate_batch(model_actions, class_representations, batch_size, episode_labels)
        else:
            next_state_start = reinforcement_learner.next_state_batch(model_actions, class_representations, batch_size)
