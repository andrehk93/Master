...
def validate(model, epoch, optimizer, test_loader, args, reinforcement_learner, episode, criterion,  multi_state=False, state_size=5, batch_size=32):
    ...
    for i_e in range(len(label_batch)):
        ...
        if args.cuda:
            q_values, hidden = model(Variable(state, volatile=True).type(torch.FloatTensor).cuda(),
                                     hidden)
        else:
            q_values, hidden = model(Variable(state, volatile=True).type(torch.FloatTensor),
                                     hidden)
        ...
        # Collect rewards:
        rewards = reinforcement_learner.collect_reward_batch(agent_actions, one_hot_labels, batch_size)

        # Collecting average reward at time t:
        episode_reward += float(sum(rewards)/batch_size)

        # Just some statistics logging:
        stats = update_dicts(batch_size, episode_labels, rewards, reinforcement_learner, label_dict,
        ...
        next_state_start = reinforcement_learner.next_state_batch(agent_actions, one_hot_labels, batch_size)
