...
# TODO: Refactor like main, train and test
def run(model, scenario_loader, batch_size, reinforcement_learner, class_vector_size, cuda, multi_state=False,
        state_size=5):
    # Initialize training:
    model.eval()
    ...
    for i_e in range(len(label_batch)):
        ...
        if (multi_state):
            next_state_start = reinforcement_learner.next_multistate_batch(agent_actions, class_representations,
                                                                           batch_size, episode_labels)
        else:
...
def get_multiclass_representations(batch_size, classes):
    ...
    bits = np.array(
        [np.array([np.array(np.random.choice(len(label_list), len(label_list), replace=True)) for c in range(classes)])
         for b in range(batch_size)])
    one_hot_vectors = np.array(
        [np.array([np.zeros((len(label_list), len(label_list))) for c in range(classes)]) for b in range(batch_size)])
    for b in range(batch_size):
